...
export interface Config extends ReviewConfig {
  ...
  /**
   * Auto-detect complex problem solving or bug fixes in memory writes
   * and automatically create knowledge cards. This bridges the memory
   * system with the knowledge card system.
   */
  autoKnowledgeFromMemory: boolean
}
...
export const Config: z<Config> = z.object({
  ...
  autoKnowledgeFromMemory: z.boolean().default(true),
...
const TOOL_DESCRIPTION = [
  ...
  'When writing about complex problem solving, bug fixes, technical decisions, or lessons learned, the system will automatically create a knowledge card (📇 auto-detected).',
].join(' ')
...
export function apply(ctx: Context, config: Config): void {
  ...
  // Knowledge store for auto-detecting complex problem solving in memory.
  // Shares the same profile directory as the knowledge plugin.
  const defaultKnowledgeConfig: KnowledgeConfig = {
    enabled: true, maxCards: 500, titleCharLimit: 80, cardCharLimit: 600,
    maxTags: 8, tagCharLimit: 24, retrieveTopK: 4, autoRetrieval: true,
  }
  const knowledgeStore = config.autoKnowledgeFromMemory
    ? new KnowledgeStore(join(ctx.desktopProfiles.current.dir, 'knowledge'), defaultKnowledgeConfig)
    : null
  ...
  ctx.effect(async () => {
    ...
    // Load knowledge store for auto-detection feature
    if (knowledgeStore !== null) {
      try {
        await knowledgeStore.loadFromDisk()
      } catch (error) {
        ctx.logger.warn('dsh-plugin-desktop: knowledge store (for auto-detection) failed to load: %s', String(error))
      }
    }
    ...
    if (config.memoryEnabled || config.userProfileEnabled) {
      ...
      try {
        disposers.push(ctx.tools.register(defineTool({
          ...
          execute: async (args: unknown, _exec) => {
            const input = args as MemoryToolArguments
            const target = normalizeTarget(input.target)
            const operations = normalizeOperations(input)
            const result = await store.applyOperations(target, operations, { origin: 'foreground' })

            // Auto-detect complex problem solving and create knowledge cards
            if (result.success && knowledgeStore !== null) {
              try {
                const knowledgeTitles = await autoKnowledgeFromMemory(knowledgeStore, operations)
                if (knowledgeTitles.length > 0) {
                  const autoMsg = ` 📇 自动沉淀知识卡：${knowledgeTitles.join('、')}`
                  return {
                    ...result,
                    message: (result.message ?? '') + autoMsg,
                  }
                }
              } catch (error) {
                // Knowledge auto-creation is best-effort; never fail the memory write
                ctx.logger.warn('dsh-plugin-desktop: auto-knowledge from memory failed: %s', String(error))
              }
            }
            return result
          },
