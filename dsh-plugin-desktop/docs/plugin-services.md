...
- GitHub release announcements (`html_url`, `name`, `body`, `published_at`) ride along with the check result. In packaged builds the confirmation and download/install flow use a **styled Hermes-style update window** (`src/update-dialog.ts`): a `⚠ DSH Desktop 更新可用` banner with `v{current} → v{next}`, a release-notes preview, and buttons for 立即更新 / 查看公告 / 稍后; downloading shows a live stage line and animated progress bar; completion offers 立即重启安装 / 稍后. Any failure to construct the styled window falls back to the native message boxes.
- Downloads stream throttled progress into the tray label (`Downloading DSH Desktop x.y.z… 42%`) and the styled progress window. Download and installer-handoff failures surface as a native **DSH Desktop Update Failed** notification instead of failing silently; the Windows hand-off is observed for a short settle window before the app quits so a dead installer spawn can be reported instead of stranding the user with no app.
