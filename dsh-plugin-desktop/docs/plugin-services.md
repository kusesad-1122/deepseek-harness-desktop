...
- GitHub release announcements (`html_url`, `name`, `body`, `published_at`) ride along with the check result. The native confirmation dialog shows the announcement text and offers a **Release Notes** button that opens the release page in the default browser.
- Downloads stream throttled progress into the tray label (`Downloading DSH Desktop x.y.z… 42%`). Download and installer-handoff failures surface as a native **DSH Desktop Update Failed** notification instead of failing silently; the Windows hand-off is observed for a short settle window before the app quits so a dead installer spawn can be reported instead of stranding the user with no app.
